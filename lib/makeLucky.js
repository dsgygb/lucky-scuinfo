...
var doNotice = function(start){
    ...
    NoticeModel.findOne({status:true,log:{$ne:"111923"}}).skip(0).limit(1).exec(function(err,doc){
        //console.log(err,doc);
       if(!doc){
           console.log('结束');
           return;
        ...
        request.post(config.noticeApiUrl+"/api/wechat/sendTemplate",{
            form: {
                first: (doc.status ? "" : "哈哈哈哈哈哈哈,")+doc.nickname + "你好," + (doc.status ? "降龙爪爪将于11月27日开业,请于11月27日-12月5日期间凭个人中奖主页抽奖码进店兑奖." : "和以前一样,你又没有中奖!点击看看谁中奖了!"),
                keyword1: "降龙爪爪套餐（2人份)",
                keyword2: "11月17日22点",
                remark: doc.status ? "另外,店主包子和我本人非常抱歉这次开业过程中出现的意外,将会额外再奉上耙耙耙鸡爪一份,口味任选。\n 希望能吃得开心,感谢支持和理解." : "也憋灰心,还是要相信这个世界,多关注scuinfo的微博和微信,scuinfo还会发更多的福利.",
                openId: doc.open_id,
                template: "prize",
            ...
            console.log(start+JSON.stringify({
                    ...
                    keyword1: "降龙爪爪套餐（2人份)",
                    keyword2: "11月17日22点",
                    remark: doc.status ? "" : "也憋灰心,还是要相信这个世界,多关注scuinfo的微博和微信,scuinfo还会发更多的福利.",
                    openId: doc.open_id,
                    template: "prize",
                    url: doc.status ?"http://lucky.scuinfo.com":"http://lucky.scuinfo.com/prize.html"
                }) +r11.body);

            NoticeModel.findByIdAndUpdate(doc._id,{log:"111923",reason:r11.body},function(ee,rr){
