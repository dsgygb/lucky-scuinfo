...
var doNotice = function(start){
    ...
    NoticeModel.findOne({status:true,log:{$ne:"111813"}}).skip(0).limit(1).exec(function(err,doc){
        //console.log(err,doc);
       if(!doc){
           console.log('结束');
           return;
        ...
        request.post(config.noticeApiUrl+"/api/wechat/sendTemplate",{
            form: {
                first: (doc.status ? "" : "哈哈哈哈哈哈哈,")+doc.nickname + "你好," + (doc.status ? "店方有紧急通知,首先恭喜你有" + doc.counts + "个组合中奖,通知详见下文:" : "和以前一样,你又没有中奖!点击看看谁中奖了!"),
                keyword1: "降龙爪爪套餐（2人份)",
                keyword2: "11月17日22点",
                remark: doc.status ? "接店家紧急通知,由于食药监局电话要求歇业临检，兑奖时间延长10天,目前还在沟通中,预计后天可以开业,有最新情况会在这个网址里及时更新.中奖情况永久有效.包子到时候也会送大家礼物补偿.感谢理解." : "也憋灰心,还是要相信这个世界,多关注scuinfo的微博和微信,scuinfo还会发更多的福利.",
                openId: doc.open_id,
                template: "prize",
                url: doc.status ?"http://lucky.scuinfo.com/prize.html":"http://lucky.scuinfo.com/prize.html"
            }
        },function(e11,r11){
            console.log(start+JSON.stringify({
                    first: (doc.status ? "紧急通知:" : "哈哈哈哈哈哈哈,")+doc.nickname + "你好," + (doc.status ? "恭喜你有" + doc.counts + "个组合中奖,详见下文" : "和以前一样,你又没有中奖!点击看看谁中奖了!"),
                    keyword1: "降龙爪爪套餐（2人份)",
                    keyword2: "11月17日22点",
                    remark: doc.status ? "接店家紧急通知,由于食药监局电话要求歇业临检，兑奖时间延长10天,目前还在沟通中,有最新情况会在这里及时更新.中奖情况永久有效.包子到时候也会送大家礼物补偿.感谢理解." : "也憋灰心,还是要相信这个世界,多关注scuinfo的微博和微信,scuinfo还会发更多的福利.",
                    openId: doc.open_id,
                    template: "prize",
                    url: doc.status ?"http://lucky.scuinfo.com":"http://lucky.scuinfo.com/prize.html"
                }) +r11.body);

            NoticeModel.findByIdAndUpdate(doc._id,{log:"111813",reason:r11.body},function(ee,rr){
